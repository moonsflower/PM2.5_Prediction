...
model = tf.keras.Sequential([
    layers.Conv1D(filters=6, kernel_size=5, padding='same', activation='relu'),
    layers.MaxPool1D(pool_size=2, strides=1),
    Dropout(0.4),
    # LSTM(80, return_sequences=True)#参数：记忆体个数，，return_sequences=Flase默认（一般最后一层用false，中间层用true）
...
checkpoint_save_path = "./checkpoint/CNN+BiGRU100.ckpt"
